import { APIRequest, ElasticBooks, Product } from '../@types';
...
export const useFetchBooks = (url: string) => {
  ...
  /*  const { result, callTx, error,loading } = useTxCallback<ElasticBooks>(
    ...
    const [data, setData] = useState<ElasticBooks>({ aggs: [], books: [] });
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.post(fullPath(url),{targetMethod: "GET"});
...
export const useBook = (id: string) => {
   /* const book = useMemo(() => books.find(book => book.id === id), [id]);
    return book;*/
    const [request] = useState<APIRequest<any>>({
        targetMethod: 'GET'
    });

    const { result, error,loading } = useTx<Product>(
        `${getBooks}/${id}`,
        request
    );

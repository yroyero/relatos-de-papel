import { APIRequest, DataResult, ElasticBooks, Product } from '../@types';
import { useMemo, useState, useEffect } from 'react';
import axios from 'axios';
import { fullPath, getBooks } from '../utils/urls';
...
export const useFetchBooks = (url: string) => {
  ...
  /*  const { result, callTx, error,loading } = useTxCallback<ElasticBooks>(
        getBooks
    );*/
 
    const [data, setData] = useState<DataResult<Product[]>>({ data: [] });
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(fullPath(url));
...
export const useBook = (id: number) => {
     ...
     const [result, setResult] = useState<DataResult<Product>>({ data: {} as Product });
    const [loading, setLoading] = useState<boolean>(true);
    const [error, setError] = useState<string | null>(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(fullPath(`${getBooks}/${id}`));
